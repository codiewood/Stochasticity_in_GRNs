...
inputs = {1,1,1,1,0,0,0,0,5,14,0.1,1e-4,1e-2,2,1e-1};
Mobj = model_4D_GRN(inputs);
[ssa_t, ssa_simdata, ssa_names] = SSA_4D_GRN(Mobj);
[ode_t, ode_simdata, ode_names] = ODE_4D_GRN(Mobj);
...
figure;
    ...
    if [inputs{1:4}] == [1,1,1,1]
        ...
        plot(ssa_t, 0.98*ssa_simdata(:,[14]), 'LineStyle', 'None', 'MarkerSize',10,'Marker','.')
        plot(ssa_t, 0.97*ssa_simdata(:,[19]), 'LineStyle', 'None', 'MarkerSize',10,'Marker','.')
        ylim([0.01 1.01])
        set(gca,'YTickLabel',[])
    end
    legend({'SSA Protein a','SSA Protein b','SSA Protein c','SSA Protein d', ...
        'ODE Protein a', 'ODE Protein b', 'ODE Protein c', 'ODE Protein d', ...
        'Promoter A Activated', 'Promoter B Activated', 'Promoter C Activated', 'Promoter D Activated'}, ...
